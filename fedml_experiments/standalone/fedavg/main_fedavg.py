...
from fedml_api.model.cv.cnn import CNN_DropOut, CNN_Slimmable, CNN_Slimmable_CIFA100
...
def add_args(parser):
    ...
    parser.add_argument('--lr', type=float, default=0.00312, metavar='LR',
                        help='learning rate (default: 0.00312)')

    parser.add_argument('--lr_decay', type=str, default='constant',
                        help='what type of lr decay')

    parser.add_argument('--eta_min', type=float, default=1e-5, metavar='LR',
                        help='minimum learning rate')

    parser.add_argument('--wd', help='weight decay parameter;', type=float, default=1e-5)
    ...
    parser.add_argument('--use_data_IS', action="store_true", default=False,
                        help='whether to use data importance sampling or not')

    parser.add_argument('--data_IS_alpha', type=float, default=0.5,
                        help='weighted factor in data importance')

    parser.add_argument('--use_client_IS', action="store_true", default=False,
                        help='whether to use client importance sampling or not')

    parser.add_argument('--stale_IS_weight', action="store_true", default=False,
                        help='whether to use staled weight for sampling or not')

    parser.add_argument('--comm_round', type=int, default=1500,
                        help='how many round of communications we shoud use')

    parser.add_argument('--frequency_of_the_test', type=int, default=100,
...
def create_model(args, model_name, output_dim):
    ...
    if model_name == "lr" and args.dataset == "mnist":
        ...
        if args.slim_training:
            model = CNN_Slimmable(False)
        else:
    ...
    elif model_name == 'cnn_cifar100' and args.slim_training:
        model = CNN_Slimmable_CIFA100()
    return model
